...
interface FormatDecimalOptionsInterface {
  digits?: number;
  minDigits?: number;
  locale?: string;
}
...
export function formatDecimal(
  value: number | null | undefined,
  options: FormatDecimalOptionsInterface = {}
) {
  const locale = options.locale || navigator.language || 'en-IN'; // Set default to Indian locale
...
export function formatCurrency(
  ...
  // Override settings to force INR currency
  const currency = "INR"; // Ensure INR is always used
  const locale = "en-IN"; // Ensure correct Indian formatting
...
// ✅ Debugging output
console.log(formatCurrency(945));    // ₹945.00
console.log(formatCurrency(2835));   // ₹2,835.00
console.log(formatPriceRange(1000, 5000)); // ₹1,000.00 - ₹5,000.00
